...
let xdemb = new Discord.RichEmbed()
        ...
        if(!message.member.hasPermission("BAN_MEMBERS") && message.author.id !== "505877805058359326") return message.channel.send("Sorry you don't have permission to use this!");

        let member = message.mentions.members.first();
        if(!member) return message.channel.send(xdemb)
        if(!member.bannable) return message.channel.send("I can't ban this user!")
        if(member.user.id === "505877805058359326") return message.channel.send("I can't ban my owner!")
